 ...
 */
export function printCalendars(calendars: Calendar[]): void {
 ...
 */
export function printEvents(events: CalendarEvent[]): void {
  if (events.length === 0) {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    const indicators = [];
    if (event.isOnlineMeeting) indicators.push('Online');
    if (event.isCancelled) indicators.push('Cancelled');
    if (event.isAllDay) indicators.push('All Day');
 ...
 */
export function printEventDetails(event: CalendarEvent, options?: HtmlToTextOptions): void {
  console.log('\n==================================================');
  ...
  if (event.body) {
    ...
    if (event.body.contentType === 'html') {
      ...
      const textContent = htmlToText(event.body.content, textOptions);
 ...
 */
function formatEventDateRange(start: Date, end: Date, isAllDay?: boolean): string {
  if (isAllDay) {
 ...
 */
function formatAttendeeStatus(status: string): string {
  switch (status) {
    case 'accepted':
      return 'Accepted';
    case 'tentatively':
