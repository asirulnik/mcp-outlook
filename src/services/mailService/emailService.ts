...
import { IAuthService, IEmailService, IFolderService, EmailAttachment } from '../interfaces';
...
export class EmailService implements IEmailService {
  ...
  async listEmails(
    ...
    try {
      ...
      if (!folderIdOrPath) {
        throw new ValidationError('Folder ID or path is required');
      }
      ...
      try {
        ...
        throw new NotFoundError(`Folder not found: ${folderIdOrPath}`);
      }
      ...
      const sortField = searchOptions?.sortBy || 'receivedDateTime';
      const sortOrder = searchOptions?.sortOrder || 'desc';
      const sortString = `${sortField} ${sortOrder}`;
      
      // Get base query parameters
      let queryParams = graphConfig.emailQueryParams(limit, searchOptions?.includeBodies || false);
   ...
   */
  async listAttachments(emailId: string, userEmail: string): Promise<EmailAttachment[]> {
    try {
      if (!userEmail) {
        throw new Error('User email is required for application permissions flow');
      }
   ...
   */
  async downloadAttachment(emailId: string, attachmentId: string, userEmail: string): Promise<EmailAttachment> {
    try {
      if (!userEmail) {
        throw new ValidationError('User email is required for application permissions flow');
      }
